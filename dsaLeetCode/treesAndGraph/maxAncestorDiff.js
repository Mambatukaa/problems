...
const maxAncestorDiffII = (root) => {
  ...
  while(stack.length) {
    ...
    if(node.left) {
      stack.push([node.left, max, min]);
    };

    if(node.right) {
      stack.push([node.right, max, min]);
    };
  };
...
console.log(maxAncestorDiffII(root));
...
1. Declare stack, min max.
2. Get node from stack and update min, max;
3. When get min, max from stack update answer using the diff of max and min.
4. Pass min, max to the children
 */
