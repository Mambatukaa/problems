...
# Time Complexity: O(n) m = unique words
# Space Complexity: O(m)
def word_count_engine(document: str) -> List[List[str]]:
    word_map = {}
    word_list = document.split()

    largest_count = 0 # for bucket sort
    ...
    counter_list = [[] for _ in range(largest_count + 1)]


    for k, value in word_map.items():
        counter_list[value].append(k)

    res = []

    for i in range(len(counter_list) - 1, 0, -1):

        for word in counter_list[i]:
